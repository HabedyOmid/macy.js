var Macy = (function () {
  ...
  $e.prototype.byCss = function (parameter, context) {
    if (context && context.querySelectorAll) {
      // Check for context and its querySelectorAll method
  ...
  function getResponsiveOptions(options) {
    var docWidth = options.useContainerForBreakpoints ? options.container.clientWidth : window.innerWidth;
  ...
  function getWidths(options) {
    ...
    if (typeof margins === 'string') {
      var tempBase = parseFloat(margins);
      unit = margins.replace(tempBase, '');
      margins = tempBase;
    }
    margins = (noOfColumns - 1) * margins / noOfColumns;
    if (unit === '%') {
      return "".concat(width - margins, "%");
    }
    return "calc(".concat(width, "% - ").concat(margins).concat(unit, ")");
  }
  ...
  function getLeftPosition(ctx, col) {
    ...
    if (typeof baseMargin === 'string') {
      var tempBase = parseFloat(baseMargin, 10);
    ...
    if (unit === '%') {
      str = "".concat(totalLeft + margin, "%");
    } else {
      str = "calc(".concat(totalLeft, "% + ").concat(margin).concat(unit, ")");
    }
    return str;
  }
  ...
  function setContainerHeight(ctx) {
    ...
    container.style.height = "".concat(largest, "px");
  }
  ...
  function shuffle(ctx, $eles) {
    ...
    foreach($eles, function (ele) {
      ...
      ele.style.top = "".concat(ctx.rows[smallest], "px");
      ele.style.left = "".concat(ctx.cols[smallest]);
  ...
  function sort(ctx, $eles) {
    ...
    foreach($eles, function (ele) {
      ...
      if (isNaN(eleHeight)) return;
      ele.style.position = 'absolute';
      ele.style.top = "".concat(ctx.rows[ctx.lastcol], "px");
      ele.style.left = "".concat(ctx.cols[ctx.lastcol]);
  ...
  var calculate = function calculate(ctx) {
    ...
    var children = [];
    if (ctx.container) {
      children = refresh ? ctx.container.children : $e(':scope > *:not([data-macy-complete="1"])', ctx.container);

      // Filter out hidden children.
      children = Array.from(children).filter(function (child) {
